...
Resize.prototype = {
	...
	Set: function(resize, side) {
		...
		addEventHandler(resize, "mousedown",
		function(e) {
			oThis._fun = _fun;
			oThis._touch = resize;
			oThis.Start(window.event || e);
...
ImgCropper.prototype = {
	...
	initialize: function(container, drag, url, width, height, options) {
		...
		//设置预览对象 
		this.View = _$(this.options.View) || null; //预览对象 
		if (this.View) {
			this.viewWidth = parseInt(this.options.viewSize);
			this.viewHeight = parseInt(this.options.viewSize);
		...
		//设置预览对象 
		this.MaxView = _$(this.options.MaxView) || null; //预览对象 
		if (this.MaxView) {
			this.maxViewWidth = parseInt(this.options.maxViewSize);
			this.maxViewHeight = parseInt(this.options.maxViewSize);
			this._MaxView = null; //预览图片对象 
			this.MaxView.style.position = "relative";
			this.MaxView.style.overflow = "hidden";
			this._MaxView = this.MaxView.appendChild(document.createElement("img"));
			this._MaxView.style.position = "absolute";
		}
	...
	PreView: function() {
		...
		var o = this.Drag,
		h = this.viewWidth,
		w = h * o.offsetWidth / o.offsetHeight;
		...
		ph = this.Height * scale,
		pw = this.Width * scale,
		pt = o.offsetTop * scale,
		pl = o.offsetLeft * scale;
		if(this._view!=null){
		...
		if(this._MaxView!=null){
			h = this.maxViewWidth,
			w = h * o.offsetWidth / o.offsetHeight;
			if (w > this.maxViewHeight) {
				w = this.maxViewHeight;
				h = w * o.offsetHeight / o.offsetWidth;
			}
			scale = h / o.offsetHeight,
			ph = this.Height * scale,
			pw = this.Width * scale,
			pt = o.offsetTop * scale,
			pl = o.offsetLeft * scale;
			var styleMaxView = this._MaxView.style;
			//设置样式 
			styleMaxView.width = pw + "px";
			styleMaxView.height = ph + "px";
			styleMaxView.top = -pt + "px ";
			styleMaxView.left = -pl + "px";
			//切割预览图 
			styleMaxView.clip = "rect(" + pt + "px " + (pl + w) + "px " + (pt + h) + "px " + pl + "px)";
		}
	}
}
